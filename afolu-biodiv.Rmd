---
title: "AFOLU and Biodiversity"
author: "Marcus Thomson"
date: '2022-02-25'
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load libraries ----
```{r, message=FALSE}
library(tidyverse)
library(raster)
library(sf)
library(maps)
library(rgdal)
library(ggpattern)
```


## Load data
### Biodiversity
Species richness for all species, and for critically endangered and vulnerable (CRENVU) species.
Source: IUCN. (2022). IUCN Redlist database. <https://www.iucnredlist.org> Retrieved: 25/02/2022.
```{r}
speciesrichness_data <- raster("data/Richness_all.tif") 
crenvu_data <- raster("data/Richness_crenvu.tif")
```

These countries are members of the [High Ambition Coalition](https://www.hacfornatureandpeople.org)
Source: HAC. (2022). HAC Members list. <https://www.hacfornatureandpeople.org/hac-members> Retrieved: 4/3/2022.
```{r}
hac_countries_map <- readr::read_csv("data/High Ambition Coalition list - HAC_members_03-2022.csv")
```


### Country political borders
Shapefiles for country-level political boundaries (borders).
Source: <https://public.opendatasoft.com/explore/dataset/world-administrative-boundaries>
NB: Should update this to use a newer method.
```{r, message=FALSE}
borders_only <- readOGR(dsn = "data/world-administrative-boundaries", layer = "world-administrative-boundaries")
```

### Pathways & historical data

Read in food/feed pathways data. This was generated previously, for another study.
```{r, show_col_types=FALSE}
pathways <- readr::read_csv("data/food_feed_pathways.csv")
pathways <- pathways %>% 
  dplyr::filter(diet_plan != "CNS-CFP") %>% 
  dplyr::mutate(diet_plan = ifelse(diet_plan == "ELA-PHD", "ELA", 
                                   ifelse(diet_plan == "FDA-SDP", "USDA", 
                                          NA)))
```

Historical data from the FAO.
Source: FAO.
```{r}
hist_fao <- readr::read_csv("data/food_feed_fao.csv")
hist_fao <- hist_fao %>%
   dplyr::rename(fao_countrycode = area_code)
```

### Map country names to ISO codes

DF to map ISO3 codes to country names.
```{r}
country_code_map <- readr::read_csv("data/country_mapper.csv")
```


## Organize pathways data
Rename and arrange historical FAO data columns to make it more alike the projected pathways data frame.
```{r}
historical_pathways <- country_code_map %>%
  dplyr::right_join(
    hist_fao, 
    by = "fao_countrycode"
  ) %>% 
  dplyr::mutate(
    diet_plan = "Hist-FAO", 
    scenario = "historical", 
    kt_yr_se = NA, 
    target_year = NA
  ) %>% 
  dplyr::select(iso_alpha3, diet_plan, scenario, food_group, year, kt_yr, kt_yr_smoothed, diff1, diff2)
```

## Crop and mask biodiversity raster
The global biodiversity raster must be mapped to countries. First make a list of extant country ISO3 codes.
```{r}
country_iso3_codes <-
  borders_only@data$iso3[!is.na(borders_only@data$iso3)]
country_iso3_codes <-
  as.character(country_iso3_codes[order(country_iso3_codes)])
country_iso3_df <-
  data.frame(index = country_iso3_codes %>% dplyr::row_number(),
             iso3 = country_iso3_codes)
```

Next, crop and then mask `biodiv_data` by individual country border polygons.
```{r}
borders_only_sub <-
  borders_only[as.character(borders_only@data$iso3) %in% country_iso3_df$iso3,]
```

The spatial polygons data frame (SPDF) `borders_only_sub` contains data for only those countries that have corresponding ISO3 codes, excluding entities that are disputed or might be double-counted.

What proxy are we using for biodiversity? If it is species richness, use `speciesrichness_data`. If it is the number of species that are critically endangered and vulnerable, use `crenvu_data`.
```{r}
biodiv_data <- crenvu_data

# used for ease of making plot labels below
if(identical(raster::values(biodiv_data), raster::values(crenvu_data))) {
  biodiv_index = "critically endangered & \nvulnerable species"
} else {
  biodiv_index = "species richness"
}

```


Loop over iso3 codes to make a list of biodiveristy rasters for each country.
```{r}
biodiv_country_rast <- list()
for (country_iso3 in country_iso3_codes) {
  biodiv_sub <-
    crop(biodiv_data, extent(borders_only_sub[borders_only_sub$iso3 == country_iso3,]))
  biodiv_country_rast[[country_iso3]] <-
    mask(biodiv_sub, borders_only_sub[borders_only_sub$iso3 == country_iso3,])
}
```


`biodiv_country_rast` is a list of rasters geospatially located within country-level border polygon perimeters. Because this is a large object for the complete list of countries, make another list with pertinent information, including statistical moments of the masked raster values. This is a much lighter object.

Extract some basic data (mean, sd, median, min, max) from each of these.
```{r}
get_raster_summary <- function(x) {
  tmp1 <- raster::values(x) %>%
    na.omit() %>%
    summary() %>%
    as.vector()
  
  tmp2 <- raster::values(x) %>%
    na.omit() %>%
    sd()
  
  tmp3 <- raster::values(x) %>%
    na.omit() %>%
    length()
  
  return(data.frame(
    names = c(
      "min",
      "first_quart",
      "median",
      "mean",
      "third_quart",
      "max",
      "sd",
      "ncount"
    ),
    biodiv = c(tmp1, tmp2, tmp3)
  ))
}

biodiv_country_moments <-
  lapply(biodiv_country_rast, FUN = get_raster_summary)
```

## Critically Endangered and Vulnerable Species Analysis
Now that the data has been ordered, proceed to analytics. First, let's make a simple list of countries by median biodiversity.
```{r}
median_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][3]
}
mean_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][4]
}
sd_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][7]
}
ncount_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][8]
}

median_tmp <- lapply(names(biodiv_country_moments), FUN = median_vals)
mean_tmp <- lapply(names(biodiv_country_moments), FUN = mean_vals)
sd_tmp <- lapply(names(biodiv_country_moments), FUN = sd_vals)
ncount_tmp <- lapply(names(biodiv_country_moments), FUN = ncount_vals)

biodiv_moments_df <- data.frame(
  iso_alpha3 = names(biodiv_country_moments),
  biodiv_median = unlist(median_tmp), 
  biodiv_mean = unlist(mean_tmp), 
  biodiv_sd = unlist(sd_tmp), 
  biodiv_ncount = unlist(ncount_tmp)
)

rm(median_tmp, mean_tmp, sd_tmp, ncount_tmp)
```

### Find pathway ROC gaps
We want to see the difference between transformation pathway and historical values; so first transformation pathways...
```{r}
trans_pathways_moments <- pathways %>%
  dplyr::group_by(iso_alpha3, diet_plan, scenario, food_group, target_year) %>%
  dplyr::filter(!is.na(diff1)) %>%
  dplyr::summarise(
    diff1_ave = mean(diff1, na.rm = TRUE),
    diff1_med = median(diff1, na.rm = TRUE),
    diff1_sdv = sd(diff1, na.rm = TRUE),
    .groups = "drop"
  )
```

...and then historical pathways.
```{r}
hist_pathways_moments <- historical_pathways %>%
  dplyr::group_by(iso_alpha3, food_group)  %>%
  dplyr::summarise(
    hist_diff1_ave = mean(diff1, na.rm = TRUE),
    hist_diff1_med = median(diff1, na.rm = TRUE),
    hist_diff1_sdv = sd(diff1, na.rm = TRUE),
    .groups = "drop"
  )
```

Next, let's take differences (delta) between long-term averages of historical (FAO) and transformation pathways.
```{r}
delta_pathways_moments <- trans_pathways_moments %>%
  dplyr::full_join(hist_pathways_moments, by = c("iso_alpha3", "food_group")) %>%
  dplyr::mutate(
    delta_diff1_ave = diff1_ave - hist_diff1_ave,
    delta_diff1_med = diff1_med - hist_diff1_med
  ) %>%
  dplyr::select(
    iso_alpha3,
    diet_plan,
    scenario,
    food_group,
    target_year,
    delta_diff1_ave,
    delta_diff1_med
  )
```

Merge these data sets.
```{r}
pathways_biodiv_moments_df <- delta_pathways_moments %>%
  dplyr::full_join(biodiv_moments_df, by = "iso_alpha3") %>%
  tidyr::drop_na()
```

`pathways_biodiv_moments_df` contains differences (deltas) between transformation and historical pathway means and medians, which I call "gaps".

### Test of gaps
Let's carry out a test to see what it looks like.
```{r}
test <- pathways_biodiv_moments_df %>%
  dplyr::filter(diet_plan == "USDA" &
                  scenario == "SSP3" &
                  food_group == "plant products")
```


HAC countries
```{r}
hac_codes <- hac_countries_map %>% 
               dplyr::pull(iso_alpha3)
```


Plot the test data. Use median biodiv index values.
```{r}
ggplot(
  test %>%
    dplyr::filter(
      diet_plan == "USDA" &
        scenario == "SSP3" &
        food_group == "plant products" & target_year == 2050
    ) %>%
    dplyr::filter(delta_diff1_ave >= 0),
  aes(x = delta_diff1_ave,
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) + 
  geom_smooth(method = "lm", colour = "firebrick", fill = "firebrick") +
  geom_point(data = test %>%
    dplyr::filter(
      diet_plan == "USDA" &
        scenario == "SSP3" &
        food_group == "plant products" & target_year == 2050
    ) %>%
    dplyr::filter(delta_diff1_ave >= 0) %>% 
      dplyr::filter(iso_alpha3 %in% hac_codes), 
    aes(x = delta_diff1_ave,
      y = biodiv_median), 
    colour = "blue",
    size = 2, 
    shape = 2) + 
  geom_smooth(data = test %>%
    dplyr::filter(
      diet_plan == "USDA" &
        scenario == "SSP3" &
        food_group == "plant products" & target_year == 2050
    ) %>%
    dplyr::filter(delta_diff1_ave >= 0) %>% 
      dplyr::filter(iso_alpha3 %in% hac_codes), 
    mapping = aes(x = delta_diff1_ave,
      y = biodiv_median), 
    method = "lm", colour = "blue", linetype = 2, fill = "blue") + 
  theme_bw() +
  labs(
    title = "Increasing pathway gaps for high biodiversity countries",
    subtitle = "filters: USDA diet, SSP3 scenario, 2050 target year for plant products only",
    x = "Difference between transformation pathway ROC and FAO historical ROC (kt/yr)",
    y = biodiv_index
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  ) + 
  annotation_logticks(sides = "b", size = 0.4)
```

### Look at historical ROCs
Historical pathways and biodiversity
```{r}
hist_moments <- hist_pathways_moments %>% 
  full_join(biodiv_moments_df, by = "iso_alpha3") %>% 
  filter(!is.na(hist_diff1_ave) & !is.na(biodiv_median) & !is.na(iso_alpha3))

```

Make some annotation label dfs to show correlation coefficients on the plot facets.
```{r}
plant_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

plant_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

corval_labs_pos = c(
  paste0(
    "r = ",
    round(plant_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_pos_corr$estimate, 2),
     " (p ~ ",
    signif(fish_prod_pos_corr$p.value, 1),
    ")"
  )
)
corval_labs_neg = c(
  paste0(
    "r = ",
    round(plant_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(fish_prod_neg_corr$p.value, 1),
    ")"
  )
)

annotate_text <- data.frame(
  food_group = c("plant products", "animal products", "fish & seafood"),
  corval_labs_pos, 
  corval_labs_neg
)
```

Plot the historical data.
```{r}
p_pos0 <- ggplot(
  data = hist_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    filter(hist_diff1_ave > 0),
  aes(x = hist_diff1_ave,
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, ymax = biodiv_median + biodiv_sd),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick",
              method = "lm",
              fill = NA) +
  theme_bw() +
  theme(
    title = element_text(size = 10),
    legend.position = "top",
    legend.title = element_blank(),
    panel.grid = element_blank()
  ) +
  labs(
    title = "High biodiveristy countries have experienced more rapid speed-ups in historical supply",
    x = "Average FAO historical ROC (kt/yr)",
    y = biodiv_index
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)

p_pos <- p_pos0 +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap(~ food_group)

p_neg0 <- ggplot(
  data = hist_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(hist_diff1_ave < 0),
  aes(x = abs(hist_diff1_ave),
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, 
        ymax = biodiv_median + biodiv_sd
        ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick",
              method = "lm",
              fill = NA) +
  theme_bw() +
  theme(
    title = element_text(size = 10),
    legend.position = "top",
    legend.title = element_blank(),
    panel.grid = element_blank()
  ) +
  labs(title = "Slow-downs in historical supply have no correlation with biodiversity",
       x = "Average FAO historical ROC (kt/yr)",
       y = biodiv_index) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)
p_neg <- p_neg0 +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_neg),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap(~ food_group) 
```


```{r, message=FALSE, warning=FALSE}
cowplot::plot_grid(p_pos, p_neg, ncol = 1)
png(filename = "~/Documents/Data/Output/hist_plots.png", width = 600, height = 400)
cowplot::plot_grid(p_pos, p_neg, ncol = 1)
dev.off()
```

This shows that large increases in the ROC of production (speed-ups) are more common in high-biodiversity countries, and decreases (down-shifts) are not correlated at all.

Is this different for countries that have signed up to the HAC pledge? Subset the HAC countries.
```{r}
hac_hist_moments <- hist_moments %>% 
  dplyr::filter(iso_alpha3 %in% hac_codes)
```

Run the correlations again for HAC countries subset.
```{r}
plant_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

plant_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

corval_labs_pos = c(
  paste0(
    "r = ",
    round(plant_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(fish_prod_pos_corr$p.value, 1),
    ")"
  )
)
corval_labs_neg = c(
  paste0(
    "r = ",
    round(plant_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(fish_prod_neg_corr$p.value, 1),
    ")"
  )
)

annotate_text <- data.frame(
  food_group = c("plant products", "animal products", "fish & seafood"),
  corval_labs_pos, 
  corval_labs_neg
)
```

## Plot Figure 1

Blue triangle-shaped points are over-plotted to represent HAC signatories.
```{r}
hist_moments_1 <- hist_moments %>%
    filter(food_group != "fish & seafood")
annotate_text_1 <- annotate_text %>%
    filter(food_group != "fish & seafood")
```


```{r}
p_pos0 <- ggplot(
  data = hist_moments_1 %>%
    filter(hist_diff1_ave > 0),
  aes(x = hist_diff1_ave,
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), size = 0.8, alpha = 0.4) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.4
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, 
        ymax = biodiv_median + biodiv_sd),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.4
  ) +
  geom_smooth(
    colour = "firebrick",
    method = "lm",
    fill = NA,
    alpha = 0.5
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11),
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank(), 
    panel.grid = element_blank()
  ) +
  labs(
    tag = "A",
    title = "Speed-ups in historical supply",
    x = "Average FAO historical ROC (kt/yr)",
    y = "Critically endangered & vulnerable\nterrestrial species"
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)

p_pos1 <- p_pos0 +
  # geom_point(
  #   data = hac_hist_moments %>%
  #     filter(food_group != "fish & seafood") %>%
  #     dplyr::filter(hist_diff1_ave > 0),
  #   aes(x = hist_diff1_ave,
  #       y = biodiv_mean),
  #   size = 2,
  #   shape = 2,
  #   colour = "blue",
  #   alpha = 0.9
  # ) +
  # geom_smooth(
  #   data = hac_hist_moments %>%
  #     filter(food_group != "fish & seafood") %>%
  #     dplyr::filter(hist_diff1_ave > 0),
  #   mapping = aes(x = hist_diff1_ave,
  #                 y = biodiv_mean),
  #   method = "lm",
  #   colour = "firebrick",
  #   linetype = "dashed", 
  #   fill = NA
  # ) +
  geom_text(
    data    = annotate_text_1 %>%
      filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  facet_wrap(~ food_group)


p_neg0 <- ggplot(
  data = hist_moments_1 %>%
    filter(food_group != "fish & seafood") %>%
    filter(hist_diff1_ave < 0),
  aes(x = abs(hist_diff1_ave),
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), size = 0.8, alpha = 0.4) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.4
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, 
        ymax = biodiv_median + biodiv_sd),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.4
  ) +
  geom_smooth(
    colour = "firebrick",
    method = "lm",
    fill = NA,
    alpha = 0.5
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11),
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank(), 
    panel.grid = element_blank()
  ) +
  labs(
    tag = "B",
    title = "Slow-downs in historical supply",
    x = "Average FAO historical ROC (kt/yr)",
    y = "Critically endangered & vulnerable\nterrestrial species"
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)

p_neg1 <- p_neg0 +
  # geom_point(
  #   data = hac_hist_moments %>%
  #     filter(food_group != "fish & seafood") %>%
  #     dplyr::filter(hist_diff1_ave < 0),
  #   aes(x = abs(hist_diff1_ave),
  #       y = biodiv_mean),
  #   size = 2,
  #   shape = 2,
  #   colour = "blue",
  #   alpha = 0.9
  # ) +
  # geom_smooth(
  #   data = hac_hist_moments %>%
  #     filter(food_group != "fish & seafood") %>%
  #     dplyr::filter(hist_diff1_ave < 0),
  #   mapping = aes(x = abs(hist_diff1_ave),
  #                 y = biodiv_mean),
  #   method = "lm", 
  #   colour = "firebrick",
  #   linetype = "dashed", 
  #   fill = NA
  # ) +
  geom_text(
    data    = annotate_text_1 %>%
      filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  facet_wrap(~ food_group)
```

The over-plotted open triangles correspond to countries that are HAC signatories. The others are not. The dashed line is the linear fit to the HAC countries; the solid line is to all countries.
```{r, message=FALSE, warning=FALSE}
cowplot::plot_grid(p_pos1, p_neg1, ncol = 2)
png(filename = "plots/figure_1-ab.jpg", width = 750, height = 250, units = "px")
cowplot::plot_grid(p_pos1, p_neg1, ncol = 2)
dev.off()
```

The correlation with speed-ups appears to go away entirely; it's still uncorrelated for down-shifts.


### Variance of results
#### ANOVA
How much does food group, diet plan and target year impact the outcome of delta_diff1_med?


<!-- Food group and diet plan are most significant.  -->
<!-- ```{r} -->
<!-- dat_tmp <- pathways_biodiv_moments_df %>%  -->
<!--   dplyr::select(-delta_diff1_med, -biodiv_median, -biodiv_sd) %>%  -->
<!--   dplyr::filter(delta_diff1_ave > 0) -->
<!-- # %>%  -->
<!-- #   dplyr::filter(iso_alpha3 == "AFG")  -->

<!-- dat_tmp$diet_plan <- factor(dat_tmp$diet_plan, levels = c("CNS-CFP","ELA-PHD","FDA-SDP")) -->
<!-- dat_tmp$scenario <- factor(dat_tmp$scenario, levels = c("SSP1","SSP2","SSP3","SSP4","SSP5")) -->
<!-- dat_tmp$food_group <- factor(dat_tmp$food_group, levels = c("animal products", "fish & seafood", "plant products")) -->
<!-- dat_tmp$target_year <- factor(dat_tmp$target_year, levels = c(2045, 2050, 2055, 2060, 2065)) -->

<!-- # anova_tmp <- aov(biodiv_mean ~ delta_diff1_ave + diet_plan + scenario + food_group + target_year + iso_alpha3, data = dat_tmp) -->
<!-- anova_tmp <- aov(delta_diff1_ave ~ diet_plan + scenario + food_group + target_year, data = dat_tmp) -->
<!-- # anova_tmp <- aov(biodiv_mean ~ delta_diff1_ave*diet_plan*scenario*target_year, data = dat_tmp) -->

<!-- anova_results <- summary(anova_tmp) -->
<!-- anova_results -->
<!-- ``` -->



<!-- TODO Figure out why the following is breaking. -->

<!-- For what countries is target year even significant? -->
<!-- ```{r} -->
<!-- anova_results_df <- data.frame(iso_alpha3 = NA, pval = NA) -->
<!-- countries <- pathways_biodiv_moments_df %>% distinct(iso_alpha3) %>% pull(iso_alpha3) -->
<!-- for(iso in countries) { -->
<!--   dat_tmp <- pathways_biodiv_moments_df %>%  -->
<!--   dplyr::select(-delta_diff1_med, -biodiv_median, -biodiv_sd) %>%  -->
<!--   dplyr::filter(delta_diff1_ave > 0) %>%  -->
<!--   dplyr::filter(iso_alpha3 == iso) %>%  -->
<!--   dplyr::mutate(target_year = as.factor(target_year)) %>%  -->
<!--   tidyr::drop_na() -->

<!--   # if(length(dat_tmp$target_year) == 0) next -->

<!--   # dat_tmp$target_year <- factor(dat_tmp$target_year, levels = c(2045, 2050, 2055, 2060, 2065)) -->
<!--     # dat_tmp$target_year <- as.factor(dat_tmp$target_year) -->
<!--   anova_tmp <- aov(biodiv_mean ~ delta_diff1_ave + diet_plan + scenario + food_group + target_year, data = dat_tmp) -->
<!--   anova_results <- summary(anova_tmp) -->

<!--   if(!is.na(unlist(anova_results)[29])) { -->
<!--      pval_tmp <- unlist(anova_results)[29] -->
<!--      names(pval_tmp) <- c() -->
<!--   } else { -->
<!--     pval_tmp <- NA -->
<!--   } -->
<!--   tmp <- data.frame( -->
<!--     iso_alpha3 = iso,  -->
<!--     pval = pval_tmp -->
<!--   ) -->
<!--   anova_results_df <- anova_results_df %>%  -->
<!--     dplyr::bind_rows(tmp) -->
<!-- } -->
<!-- rownames(anova_results_df) <- c() -->

<!-- anova_results_df <- anova_results_df %>%  -->
<!--   tidyr::drop_na(iso_alpha3) %>%  -->
<!--   dplyr::arrange(pval) -->
<!-- ``` -->

<!-- Which countries show significant results? -->
<!-- ```{r} -->
<!-- anova_results_df %>% -->
<!--   filter(pval <= 0.05) -->
<!-- ``` -->


#### PCA test
<!-- Recode character columns as numeric. -->
<!-- ```{r} -->
<!-- diet_plans <- data.frame( -->
<!--   diet_plan = c("CNS-CFP","ELA-PHD","FDA-SDP"), -->
<!--   ndiets = c(1,2,3) -->
<!-- ) -->
<!-- food_groups <- data.frame( -->
<!--   food_group = c("animal products", "fish & seafood", "plant products"), -->
<!--   nfoods = c(1,2,3) -->
<!-- ) -->
<!-- ssp_scenarios <- data.frame( -->
<!--   scenario = c("SSP1","SSP2","SSP3","SSP4","SSP5"), -->
<!--   nscen = c(1,2,3,4,5) -->
<!-- ) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- recoded_pathways_df <- pathways %>% -->
<!--   dplyr::left_join(diet_plans, by = "diet_plan") %>% -->
<!--   dplyr::left_join(food_groups, by = "food_group") %>% -->
<!--   dplyr::left_join(ssp_scenarios, by = "scenario") %>%  -->
<!--   dplyr::select(diff1, target_year, ndiets, nfoods, nscen) -->
<!-- ``` -->


<!-- ```{r} -->
<!-- recoded_pathways_df <- pathways_biodiv_moments_df %>% -->
<!--   dplyr::left_join(diet_plans, by = "diet_plan") %>% -->
<!--   dplyr::left_join(food_groups, by = "food_group") %>% -->
<!--   dplyr::left_join(ssp_scenarios, by = "scenario") %>% -->
<!--   dplyr::select(-diet_plan, -scenario, -food_group, -delta_diff1_med, -biodiv_median, -biodiv_sd) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- tmp <- recoded_pathways_df %>% -->
<!--   # dplyr::filter(iso_alpha3 == "AFG") %>% -->
<!--   dplyr::select(-iso_alpha3) -->
<!-- ``` -->

<!-- Put this aside for the moment. It's not necessary. -->
<!-- ```{r} -->
<!-- pca_tmp <- prcomp(na.omit(recoded_pathways_df), center = TRUE, scale. = TRUE) -->
<!-- pca_tmp -->
<!-- summary(pca_tmp) -->
<!-- ``` -->


- intensification or imports
- order of magnitude more difficult to meet food commitments than increase biodiversity

- steepness of curves (general population growth, target date) how does the slope of the regression change as a function of target date
- what countries does target date matter, what  countries doesn't it matter?
- basket we want to know: high biodiversity, target date matters, rank these

- 2 d coordinate system: how high is its biodiversity, how hard is it for country to meet it s food needs
- make a conceptual diagram with a model country that epitomizes each of these quadrants 
- PCR test to see what the variance around each term is

### Dependence of slopes with target year
```{r}
pathways_biodiv_moments_em <- pathways_biodiv_moments_df %>% 
  dplyr::group_by(iso_alpha3, diet_plan, food_group, target_year) %>% 
  dplyr::summarise(delta_diff1_em = mean(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mn = min(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mx = max(delta_diff1_ave, na.rm = TRUE), 
                   # biodiv_em = mean(biodiv_mean, na.rm = TRUE), 
                   biodiv_em = mean(biodiv_median, na.rm = TRUE), 
                   biodiv_mn = biodiv_em - mean(biodiv_sd, na.rm = TRUE), 
                   biodiv_mx = biodiv_em + mean(biodiv_sd, na.rm = TRUE), 
                   .groups = "drop")
```


```{r}
diets <- pathways_biodiv_moments_em %>% 
  distinct(diet_plan) %>% 
  pull(diet_plan)
foods <- pathways_biodiv_moments_em %>% 
  distinct(food_group) %>% 
  pull(food_group)
```


How do the slopes of the regression lines change as a function of target year?
```{r, message=FALSE, echo=FALSE}
slopes_df <-
  data.frame(
    diet_plan = NA,
    food_group = NA,
    target_year = NA,
    slope = NA,
    dROC = NA
  )
for (diet in diets) {
  for (food in foods) {
    p_path_pos0 <- ggplot(
      data = pathways_biodiv_moments_em %>%
        dplyr::filter(delta_diff1_em > 0) %>%
        dplyr::filter(diet_plan == diet &
                        food_group == food),
      aes(x = delta_diff1_em,
          y = biodiv_em)
    ) +
      geom_point(colour = scales::muted("blue"), alpha = 0.5) +
      geom_smooth(colour = "firebrick", method = "lm") +
      scale_x_log10(
        breaks = scales::trans_breaks("log10", function(x)
          10 ^ x),
        labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
      )
    p_path_neg0 <- ggplot(
      data = pathways_biodiv_moments_em %>%
        dplyr::filter(delta_diff1_em < 0) %>%
        dplyr::filter(diet_plan == diet &
                        food_group == food),
      aes(x = abs(delta_diff1_em),
          y = biodiv_em)
    ) +
      geom_point(colour = scales::muted("blue"), alpha = 0.5) +
      geom_smooth(colour = "firebrick", method = "lm") +
      scale_x_log10(
        breaks = scales::trans_breaks("log10", function(x) {
          tmp <- abs(x)
          10 ^ tmp
        }),
        labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                     .x))
      )
    
    p_path_pos <- p_path_pos0 +
      facet_wrap( ~ target_year, ncol = 5)
    p_path_neg <- p_path_neg0 +
      facet_wrap( ~ target_year, ncol = 5)
    
    p_path_pos_dat <- ggplot_build(p_path_pos)
    p_path_neg_dat <- ggplot_build(p_path_neg)
    
    pathways_fit_pos_df <- p_path_pos_dat$data[[2]]
    slopes_by_target_year_pos <- pathways_fit_pos_df %>%
      dplyr::group_by(PANEL) %>%
      dplyr::mutate(dx = x - lag(x, 1),
                    dy = y - lag(y, 1)) %>%
      dplyr::summarise(dx = mean(dx, na.rm = TRUE),
                       dy = mean(dy, na.rm = TRUE)) %>%
      dplyr::mutate(slope = dy / dx,
                    target_year = ifelse(PANEL == 1, 2045,
                                         ifelse(
                                           PANEL == 2, 2050,
                                           ifelse(PANEL == 3, 2055,
                                                  ifelse(PANEL == 4, 2060,
                                                         2065))
                                         ))) %>%
      dplyr::ungroup() %>%
      dplyr::select(target_year, slope)
    
    pathways_fit_neg_df <- p_path_neg_dat$data[[2]]
    slopes_by_target_year_neg <- pathways_fit_neg_df %>%
      dplyr::group_by(PANEL) %>%
      dplyr::mutate(dx = x - lag(x, 1),
                    dy = y - lag(y, 1)) %>%
      dplyr::summarise(dx = mean(dx, na.rm = TRUE),
                       dy = mean(dy, na.rm = TRUE)) %>%
      dplyr::mutate(slope = dy / dx,
                    target_year = ifelse(PANEL == 1, 2045,
                                         ifelse(
                                           PANEL == 2, 2050,
                                           ifelse(PANEL == 3, 2055,
                                                  ifelse(PANEL == 4, 2060,
                                                         2065))
                                         ))) %>%
      dplyr::ungroup() %>%
      dplyr::select(target_year, slope)
    
    pos_tmp <- data.frame(
      diet_plan = diet,
      food_group = food,
      slopes_by_target_year_pos,
      dROC = "positive"
    )
    neg_tmp <- data.frame(
      diet_plan = diet,
      food_group = food,
      slopes_by_target_year_neg,
      dROC = "negative"
    )
    tmp <- pos_tmp %>%
      dplyr::bind_rows(neg_tmp)
    
    rm(pos_tmp, neg_tmp)
    
    slopes_df <- slopes_df %>%
      dplyr::bind_rows(tmp) %>%
      tidyr::drop_na()
    rm(tmp)
  }
}
```

Normalize by initial target year (2045) value.
```{r}
slopes_df <- slopes_df %>% 
  dplyr::group_by(diet_plan, food_group, target_year, dROC) %>% 
  dplyr::mutate(slope_init = ifelse(target_year == 2045, 
                                    slope, 
                                    NA)) %>% 
  dplyr::ungroup() %>% 
  tidyr::fill(slope_init, .direction = "down") %>% 
  dplyr::mutate(slope_norm = slope/slope_init)
  # dplyr::sumamrise(slope )
```


Plotting this roughly shows that the slope stays pretty constant until 2055, after which it drops.
```{r}
#plot the positive ROCs
ggplot(
  data = slopes_df %>%
    dplyr::filter(dROC == "positive"),
  mapping = aes(x = target_year,
                y = slope_norm)) + 
  geom_hline(yintercept = 1, alpha = 0.8) + 
    geom_line(aes(colour = diet_plan)) +
  theme_bw() +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
    hjust = 0.9
  )) +
  labs(x = "shift target year",
       y = "Norm. CRENVU/pathway t food-feed demand/yr") +
  facet_grid( ~ food_group)

#plot the negative ROCs
ggplot(data = slopes_df %>% 
         dplyr::filter(dROC == "negative"), 
       mapping = aes(x = target_year,
                y = slope_norm)) + 
  geom_hline(yintercept = 1, alpha = 0.8) +
    geom_line(aes(colour = diet_plan)) + 
  theme_bw() + 
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.9)
  ) + 
  labs(
    x = "shift target year", 
    y = "Norm. CRENVU/pathway t food-feed demand/yr"
  ) + 
  facet_grid(~ food_group)
```

### Plot for future ROC

```{r}
trans_path_biodiv_moments <- trans_pathways_moments %>%
  dplyr::full_join(biodiv_moments_df, by = "iso_alpha3") %>%
  tidyr::drop_na() %>%
  dplyr::group_by(iso_alpha3, diet_plan, food_group) %>%
  dplyr::summarise(
    diff1_em = mean(diff1_ave, na.rm = TRUE),
    diff1_mn = min(diff1_ave, na.rm = TRUE),
    diff1_mx = max(diff1_ave, na.rm = TRUE),
    # biodiv_em = mean(biodiv_mean, na.rm = TRUE),
    biodiv_em = mean(biodiv_median, na.rm = TRUE),
    biodiv_mn = biodiv_em - mean(biodiv_sd, na.rm = TRUE),
    biodiv_mx = biodiv_em + mean(biodiv_sd, na.rm = TRUE),
    .groups = "drop"
  )
```


```{r}
annotate_text <- data.frame()
for (food in foods) {
  for (diet in diets) {
    for (droc_sign in c(1, -1)) {
      if (droc_sign == 1) {
        pathways_tmp <-  trans_path_biodiv_moments %>%
          filter(diff1_em > 0)
      } else {
        pathways_tmp <-  trans_path_biodiv_moments %>%
          filter(diff1_em < 0)
      }
      tmp_cor <- cor.test(
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          mutate(diff1_em = log(abs(diff1_em))) %>%
          pull(diff1_em),
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          pull(biodiv_em)
      )
      tmp_df <- data.frame(
        food_group = food,
        diet_plan = diet, 
        droc_sign = droc_sign,
        corr = as.numeric(tmp_cor$estimate),
        pval = as.numeric(tmp_cor$p.value),
        labz = paste0(
          "r = ",
          round(as.numeric(tmp_cor$estimate), 2),
          " (p ~ ",
          signif(as.numeric(tmp_cor$p.value), 1),
          ")"
        )
      )
      annotate_text <- annotate_text %>%
        dplyr::bind_rows(tmp_df) %>%
        tidyr::drop_na()
    }
  }
}
```

## Plot Figure 2
```{r}
p_path_pos0 <- ggplot(
  data = trans_path_biodiv_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(diff1_em > 0),
  aes(x = diff1_em,
      y = biodiv_em)
) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"),
             size = 0.8,
             alpha = 0.5) +
  geom_errorbar(
    aes(xmin = diff1_mn,
        xmax = diff1_mx),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
  # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(
    colour = scales::muted("firebrick"),
    method = "lm",
    fill = NA,
    alpha = 0.8
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank(), 
    panel.grid = element_blank()
  ) +
  labs(tag = "A",
       title = "Speed-ups in present supply to future demand",
       x = "Present supply to future demand pathway ROC (kt/yr)",
       y = "Critically endangered & vulnerable\nterrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
  ) +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(droc_sign > 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) + 
  annotation_logticks(sides = "b", size = 0.4)
p_path_pos <- p_path_pos0 +
  facet_grid(diet_plan ~ food_group)

p_path_neg0 <- ggplot(
  data = trans_path_biodiv_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(diff1_em < 0),
  aes(x = abs(diff1_em),
      y = biodiv_em)
) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"),
             size = 0.8,
             alpha = 0.5) +
  geom_errorbar(
    aes(xmin = diff1_mn,
        xmax = diff1_mx),
    colour = scales::muted("blue"),
  # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
  # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(
    colour = scales::muted("firebrick"),
    method = "lm",
    fill = NA,
    alpha = 0.8
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank(), 
    panel.grid = element_blank()
  ) +
  labs(tag = "B",
       title = "Slow-downs in present supply to future demand",
       x = "Present supply to future demand pathway ROC (kt/yr)",
       y = "Critically endangered & vulnerable\nterrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(droc_sign < 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) + 
  annotation_logticks(sides = "b", size = 0.4)
p_path_neg <- p_path_neg0 +
  facet_grid(diet_plan ~ food_group)
```


```{r}
cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
jpeg(filename = "plots/figure_1-cd.jpg", width = 750, height = 350, units = "px")
cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
dev.off()
```

Glom figs together.

```{r}
p_path_pos <- p_path_pos0 +
  labs(
    tag = "C"
  ) + 
  facet_grid(diet_plan ~ food_group)

p_path_neg <- p_path_neg0 +
  labs(
    tag = "D"
  ) + 
  facet_grid(diet_plan ~ food_group)

p1 <- cowplot::plot_grid(p_pos1, p_neg1, ncol = 2) + 
  theme(
    plot.background = element_rect(colour = "black", size = 2)
  )
p2 <- cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2) + 
  theme(
    plot.background = element_rect(colour = "black", fill = "grey", size = 2)
  )

```

```{r}
p_fig1 <- cowplot::plot_grid(p1, p2, ncol = 1, rel_heights = c(0.4,0.6))
p_fig1
jpeg(filename = "plots/figure_1.jpg", width = 800, height = 550, units = "px")
p_fig1
dev.off()
```


# Pathways Gap Analysis

Determine the gaps, or differences between, future pathway and historical ROCs.

```{r}

tmp1 <- trans_path_biodiv_moments %>% 
  dplyr::filter(diff1_em > 0) %>% 
  dplyr::group_by(diet_plan, food_group) %>% 
  dplyr::summarise(diff1_em_mean = mean(diff1_em, na.rm = TRUE), 
                   .groups = "drop") %>% 
  dplyr::mutate(roc_sign = "positive") 

tmp2 <- trans_path_biodiv_moments %>% 
  dplyr::filter(diff1_em < 0) %>% 
  dplyr::group_by(diet_plan, food_group) %>% 
  dplyr::summarise(diff1_em_mean = mean(diff1_em, na.rm = TRUE), 
                   .groups = "drop") %>% 
  dplyr::mutate(roc_sign = "negative") 

dist_means <- tmp1 %>% 
  dplyr::bind_rows(tmp2)

```


## Plot Figure 3

Differences between the distributions indicate gaps.

```{r}
p_path_dens_pos <- ggplot(data = trans_path_biodiv_moments %>%
                            dplyr::filter(food_group != "fish & seafood") %>%
                            dplyr::filter(diff1_em > 0)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_density(
    aes(x = diff1_em),
    fill = scales::muted("blue"),
    alpha = 0.4,
    colour = NA
  ) +
  geom_density(
    data = hist_pathways_moments %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(hist_diff1_ave > 0),
    mapping = aes(x = hist_diff1_ave),
    fill = scales::muted("firebrick"),
    alpha = 0.4,
    colour = NA
  ) +
  theme_bw() +
  labs(tag = "A",
       title = "Positive rates of change",
       x = "Pathway ROC (kt/yr)",
       y = "Normalized density") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  ) +
  annotation_logticks(sides = "b", size = 0.4) +
  facet_grid(diet_plan ~ food_group)

p_path_dens_neg <- ggplot(data = trans_path_biodiv_moments %>%
                            dplyr::filter(food_group != "fish & seafood") %>%
                            dplyr::filter(diff1_em < 0)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_density(
    aes(x = abs(diff1_em)),
    fill = scales::muted("blue"),
    alpha = 0.4,
    colour = NA
  ) +
  geom_density(
    data = hist_pathways_moments %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(hist_diff1_ave < 0),
    mapping = aes(x = abs(hist_diff1_ave)),
    fill = scales::muted("firebrick"),
    alpha = 0.4,
    colour = NA
  ) +
  theme_bw() +
  labs(tag = "B",
       title = "Negative rates of change",
       x = "Pathway ROC (kt/yr)",
       y = "Normalized density") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  annotation_logticks(sides = "b", size = 0.4) +
  facet_grid(diet_plan ~ food_group)

#dummy plot to get a legend
p_0 <-
  ggplot(data = data.frame(
    category = c("historical", "future"),
    x = c(1, 1),
    y = c(1, 1)
  ),
  mapping = aes(x = 1, y = 1)) +
  geom_area(aes(fill = category),
            alpha = 0.5) +
  scale_fill_manual(
    labels = c("historical data", "future pathway"),
    values = c(scales::muted("firebrick"), scales::muted("blue"))
  ) +
  theme(
    legend.position = "top",
    legend.title = element_blank(),
    legend.text = element_text(size = 12)
  )

p_dens_legend <- cowplot::get_legend(p_0)

```


Write to disk.
```{r}


p_dens_plots <- cowplot::plot_grid(p_path_dens_pos, p_path_dens_neg, ncol = 2)
cowplot::plot_grid(p_dens_legend, p_dens_plots, ncol = 1, rel_heights = c(0.1,0.9))

jpeg(filename = "plots/figure_2.jpg", width = 750, height = 350, units = "px")
cowplot::plot_grid(p_dens_legend, p_dens_plots, ncol = 1, rel_heights = c(0.1,0.9))
dev.off()
```


Compare whether these distributions are alike, with Kolmogrov-Smirnov tests.
```{r}
set.seed(123)
```

Fugly but it works.
```{r}
# gab positive pathway data & join
pos_path_tmp <- trans_path_biodiv_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(diff1_em > 0) %>%
  dplyr::select(diet_plan, food_group, diff1_em) 
pos_hist_tmp <- hist_pathways_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(hist_diff1_ave > 0) %>%
  dplyr::select(food_group, hist_diff1_ave)
pos_hist_path_tmp <- pos_path_tmp %>% 
  dplyr::full_join(pos_hist_tmp)

# now grap negative pathway data & join
neg_path_tmp <- trans_path_biodiv_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(diff1_em < 0) %>%
  dplyr::select(diet_plan, food_group, diff1_em) %>% 
  dplyr::mutate(diff1_em = abs(diff1_em))
neg_hist_tmp <- hist_pathways_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(hist_diff1_ave < 0) %>%
  dplyr::select(food_group, hist_diff1_ave) %>% 
  dplyr::mutate(hist_diff1_ave = abs(hist_diff1_ave))
neg_hist_path_tmp <- neg_path_tmp %>% 
  dplyr::full_join(neg_hist_tmp)

for(prod in c("animal products", "plant products")) {
  for (diet in c("ELA", "USDA")) {
    tmp1 <- pos_hist_path_tmp %>%
      dplyr::filter(food_group == prod & diet_plan == diet) %>%
      dplyr::pull(diff1_em)
    tmp2 <- pos_hist_path_tmp %>%
      dplyr::filter(food_group == prod & diet_plan == diet) %>%
      dplyr::pull(hist_diff1_ave)
    
    pos_ks_test <- ks.test(tmp1, tmp2)
    rm(tmp1, tmp2)
    
    pos_ks_test_df <- data.frame(
      sign = "positive", 
      food_group = prod, 
      diet_plan = diet, 
      ks_test_statistic = pos_ks_test$statistic,
      ks_test_pval = pos_ks_test$p.value
    )
    
    tmp1 <- neg_hist_path_tmp %>%
      dplyr::filter(food_group == prod & diet_plan == diet) %>%
      dplyr::pull(diff1_em)
    tmp2 <- neg_hist_path_tmp %>%
      dplyr::filter(food_group == prod & diet_plan == diet) %>%
      dplyr::pull(hist_diff1_ave)
    
    neg_ks_test <- ks.test(tmp1, tmp2)
    rm(tmp1, tmp2)
    
    neg_ks_test_df <- data.frame(
      sign = "negative", 
      food_group = prod, 
      diet_plan = diet, 
      ks_test_statistic = neg_ks_test$statistic,
      ks_test_pval = neg_ks_test$p.value
    )
    
    ks_test_df <- pos_ks_test_df %>% 
      dplyr::bind_rows(neg_ks_test_df)
    
    if(prod == "animal products" & diet == "ELA") {
      ks_test_DF <- ks_test_df
    } else {
      ks_test_DF <- ks_test_DF %>% 
      dplyr::full_join(ks_test_df)
    }
  }
}
# tmp1 <- pos_hist_path_tmp %>%
#   dplyr::filter(food_group == "animal products" & diet_plan == "USDA") %>%
#   dplyr::pull(diff1_em)
# tmp2 <- pos_hist_path_tmp %>%
#   dplyr::filter(food_group == "animal products" & diet_plan == "USDA") %>%
#   dplyr::pull(hist_diff1_ave)
# 
# tmp <- ks.test(tmp1, tmp2)

ks_test_DF

```

Take a look at some differences.
```{r}
trans_path_biodiv_moments %>%
         dplyr::filter(food_group != "fish & seafood") %>%
         dplyr::group_by(iso_alpha3, diet_plan)
```


### Plot for future ROC gap

```{r}
pathways_biodiv_moments_em <- pathways_biodiv_moments_df %>%
  dplyr::group_by(iso_alpha3, diet_plan, food_group) %>%
  dplyr::summarise(
    delta_diff1_em = mean(delta_diff1_ave, na.rm = TRUE),
    delta_diff1_mn = min(delta_diff1_ave, na.rm = TRUE),
    delta_diff1_mx = max(delta_diff1_ave, na.rm = TRUE),
    # biodiv_em = mean(biodiv_mean, na.rm = TRUE),
    biodiv_em = mean(biodiv_median, na.rm = TRUE),
    biodiv_mn = biodiv_em - mean(biodiv_sd, na.rm = TRUE),
    biodiv_mx = biodiv_em + mean(biodiv_sd, na.rm = TRUE),
    .groups = "drop"
  )
```


This time I'm going to make the annotation labels in a df including the various diets and food groups.
```{r}
annotate_text <- data.frame()
for (food in foods) {
  for (diet in diets) {
    for (droc_sign in c(1, -1)) {
      if (droc_sign == 1) {
        pathways_tmp <-  pathways_biodiv_moments_em %>%
          filter(delta_diff1_em > 0)
      } else {
        pathways_tmp <-  pathways_biodiv_moments_em %>%
          filter(delta_diff1_em < 0)
      }
      tmp_cor <- cor.test(
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          mutate(delta_diff1_em = log(abs(delta_diff1_em))) %>%
          pull(delta_diff1_em),
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          pull(biodiv_em)
      )
      tmp_df <- data.frame(
        food_group = food,
        diet_plan = diet, 
        droc_sign = droc_sign,
        corr = as.numeric(tmp_cor$estimate),
        pval = as.numeric(tmp_cor$p.value),
        labz = paste0(
          "r = ",
          round(as.numeric(tmp_cor$estimate), 2),
          " (p ~ ",
          signif(as.numeric(tmp_cor$p.value), 1),
          ")"
        )
      )
      annotate_text <- annotate_text %>%
        dplyr::bind_rows(tmp_df) %>%
        tidyr::drop_na()
    }
  }
}
```

## Plot Figure 4

Biodiversity index plotted as a function of pathway gaps.

```{r}
p_path_pos0 <- ggplot(
  data = pathways_biodiv_moments_em %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(delta_diff1_em > 0),
  aes(x = delta_diff1_em,
      y = biodiv_em)
) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("purple"),
             size = 0.8,
             alpha = 0.5) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("purple"),
    width = .2,
    alpha = 0.4
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("purple"),
    width = .2,
    alpha = 0.4
  ) +
  geom_smooth(
    colour = scales::muted("firebrick"),
    method = "lm",
    fill = NA,
    alpha = 0.8
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank()
  ) +
  labs(tag = "A",
       title = "Speed-ups in pathway gap",
       x = "Difference between future pathway and historical ROC (kt/yr)",
       y = "Critically endangered & vulnerable terrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
  ) +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(droc_sign > 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  coord_cartesian(xlim = c(1e-3, 1e5)) + 
  annotation_logticks(sides = "b", size = 0.4)
p_path_pos <- p_path_pos0 +
  facet_grid(diet_plan ~ food_group)

p_path_neg0 <- ggplot(
  data = pathways_biodiv_moments_em %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(delta_diff1_em < 0),
  aes(x = abs(delta_diff1_em),
      y = biodiv_em)
) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("purple"),
             size = 0.8,
             alpha = 0.4) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("purple"),
    width = .2,
    alpha = 0.4
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("purple"),
    width = .2,
    alpha = 0.5
  ) +
  geom_smooth(
    colour = scales::muted("firebrick"),
    method = "lm",
    fill = NA,
    alpha = 0.8
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank()
  ) +
  labs(tag = "B",
       title = "Slow-downs in pathway gap",
       x = "Difference between future pathway and historical ROC (kt/yr)",
       y = "Critically endangered & vulnerable terrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(droc_sign < 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 4
  ) +
  coord_cartesian(xlim = c(1e-3, 1e5)) + 
  annotation_logticks(sides = "b", size = 0.4)
p_path_neg <- p_path_neg0 +
  facet_grid(diet_plan ~ food_group)
```


```{r}
cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
jpeg(filename = "plots/figure_3.jpg", width = 800, height = 350, units = "px")
cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
dev.off()
```


Now look at just HAC signatories, and then countries that are not HAC signatories.
```{r, message=FALSE}
annotate_text_hac_only <- data.frame(
  food_group = NA,
  diet_plan = NA,
  droc_sign = NA,
  corr = NA,
  pval = NA,
  labz = NA
)

for (food in c("animal products", "plant products")) {
  for (diet in diets) {
    for (droc_sign in c(1, -1)) {
      if (droc_sign == 1) {
        pathways_tmp <-  pathways_biodiv_moments_em %>% 
          filter(food_group != "fish & seafood") %>% 
          filter(iso_alpha3 %in% hac_codes) %>% 
          filter(delta_diff1_em > 0)
      } else {
        pathways_tmp <-  pathways_biodiv_moments_em %>% 
          filter(food_group != "fish & seafood") %>% 
          filter(iso_alpha3 %in% hac_codes) %>%
          filter(delta_diff1_em < 0)
      }
      tmp_cor <- cor.test(
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          mutate(delta_diff1_em = log(abs(delta_diff1_em))) %>%
          pull(delta_diff1_em),
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          pull(biodiv_em)
      )
      tmp_df <- data.frame(
        food_group = food,
        diet_plan = diet, 
        droc_sign = droc_sign,
        corr = as.numeric(tmp_cor$estimate),
        pval = as.numeric(tmp_cor$p.value),
        labz = paste0(
          "r = ",
          round(as.numeric(tmp_cor$estimate), 2),
          " (p ~ ",
          signif(as.numeric(tmp_cor$p.value), 1),
          ")"
        )
      )
      annotate_text_hac_only <- annotate_text_hac_only %>%
        dplyr::full_join(tmp_df) %>%
        tidyr::drop_na()
    }
  }
}

# non-HAC
annotate_text_non_hac_only <- data.frame(
  food_group = NA,
  diet_plan = NA,
  droc_sign = NA,
  corr = NA,
  pval = NA,
  labz = NA
)

for (food in c("animal products", "plant products")) {
  for (diet in diets) {
    for (droc_sign in c(1, -1)) {
      if (droc_sign == 1) {
        pathways_tmp <-  pathways_biodiv_moments_em %>% 
          filter(food_group != "fish & seafood") %>% 
          filter(!iso_alpha3 %in% hac_codes) %>% 
          filter(delta_diff1_em > 0)
      } else {
        pathways_tmp <-  pathways_biodiv_moments_em %>% 
          filter(food_group != "fish & seafood") %>% 
          filter(!iso_alpha3 %in% hac_codes) %>%
          filter(delta_diff1_em < 0)
      }
      tmp_cor <- cor.test(
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          mutate(delta_diff1_em = log(abs(delta_diff1_em))) %>%
          pull(delta_diff1_em),
        pathways_tmp %>%
          filter(food_group == food & diet_plan == diet) %>%
          pull(biodiv_em)
      )
      tmp_df <- data.frame(
        food_group = food,
        diet_plan = diet, 
        droc_sign = droc_sign,
        corr = as.numeric(tmp_cor$estimate),
        pval = as.numeric(tmp_cor$p.value),
        labz = paste0(
          "r = ",
          round(as.numeric(tmp_cor$estimate), 2),
          " (p ~ ",
          signif(as.numeric(tmp_cor$p.value), 1),
          ")"
        )
      )
      annotate_text_non_hac_only <- annotate_text_non_hac_only %>%
        dplyr::full_join(tmp_df) %>%
        tidyr::drop_na()
    }
  }
}

```

```{r}
tmp1 <- annotate_text %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::mutate(group = "All countries")

tmp2 <- annotate_text_hac_only %>%
  dplyr::mutate(group = "HAC only")

tmp3 <- annotate_text_non_hac_only %>%
  dplyr::mutate(group = "Non-HAC only")

hac_correlations <- tmp1 %>% 
  dplyr::full_join(tmp2) %>% 
  dplyr::full_join(tmp3)
```

## Plot Figure 5

```{r}
p_hac_pos <- ggplot(hac_correlations %>% 
         dplyr::filter(droc_sign == 1), 
       aes(x = group, 
           y = corr)) + 
  geom_col(alpha = 0.2, 
           colour = "black") + 
  geom_col_pattern(data = hac_correlations %>% 
         dplyr::filter(droc_sign == 1 & pval <= 0.1), 
         aes(x = group, 
             y = corr), 
         pattern = "stripe", 
         pattern_angle = 45, 
         alpha = 0.2, 
           colour = "black") + 
  # geom_col_pattern(data = hac_correlations %>% 
  #        dplyr::filter(droc_sign == 1 & pval <= 0.05), 
  #        aes(x = group, 
  #            y = corr), 
  #        pattern = "stripe", 
  #        pattern_angle = 90, 
  #        alpha = 0.2, 
  #          colour = "black") + 
  geom_col_pattern(data = hac_correlations %>% 
         dplyr::filter(droc_sign == 1 & pval <= 0.01), 
         aes(x = group, 
             y = corr), 
         pattern = "stripe", 
         pattern_angle = 135, 
         alpha = 0.2, 
           colour = "black")  + 
  geom_hline(yintercept = 0, alpha = 0.8) + 
  theme_bw() + 
  theme(
    axis.title.y = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "none", 
    axis.title.x = element_blank(), 
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 10), 
    axis.text.y = element_text(size = 10), 
    panel.grid.major.x = element_blank(), 
    panel.grid.minor.x = element_blank()
  ) + 
  labs(
    tag = "A", 
    title = "For speed-ups in pathway gap", 
    y = "Pearson's correlation coefficient"
  ) + 
  facet_grid(diet_plan ~ food_group)

p_hac_neg <- ggplot(hac_correlations %>% 
         dplyr::filter(droc_sign == -1), 
       aes(x = group, 
           y = corr)) + 
  geom_col(alpha = 0.2, 
           colour = "black") + 
  geom_col_pattern(data = hac_correlations %>% 
         dplyr::filter(droc_sign == -1 & pval <= 0.1), 
         aes(x = group, 
             y = corr), 
         pattern = "stripe", 
         pattern_angle = 45, 
         alpha = 0.2, 
           colour = "black") + 
  # geom_col_pattern(data = hac_correlations %>% 
  #        dplyr::filter(droc_sign == -1 & pval <= 0.05), 
  #        aes(x = group, 
  #            y = corr), 
  #        pattern = "stripe", 
  #        pattern_angle = 90, 
  #        alpha = 0.2, 
  #          colour = "black") + 
  geom_col_pattern(data = hac_correlations %>% 
         dplyr::filter(droc_sign == -1 & pval <= 0.01), 
         aes(x = group, 
             y = corr), 
         pattern = "stripe", 
         pattern_angle = 135, 
         alpha = 0.2, 
           colour = "black")  + 
  geom_hline(yintercept = 0, alpha = 0.8) + 
  theme_bw() + 
  theme(
    axis.title.y = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "none", 
    axis.title.x = element_blank(), 
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 10), 
    axis.text.y = element_text(size = 10), 
    panel.grid.major.x = element_blank(), 
    panel.grid.minor.x = element_blank()
  ) + 
  labs(
    tag = "B", 
    title = "For slow-downs in pathway gap", 
    y = "Pearson's correlation coefficient"
  ) + 
  facet_grid(diet_plan ~ food_group)

```

Write to disk.

```{r}
cowplot::plot_grid(p_hac_pos, p_hac_neg, ncol = 2)
jpeg(filename = "plots/figure_4.jpg", width = 800, height = 350, units = "px")
cowplot::plot_grid(p_hac_pos, p_hac_neg, ncol = 2)
dev.off()
```


# High species richness countries

### Look at historical ROCs

- Previously we used all biodiv index numbers for countries. What happens if we cut countries with low crenvu species?

- species richness normalized by country size
```{r}
biodiv_data <- speciesrichness_data

# used for ease of making plot labels below
if(identical(raster::values(biodiv_data), raster::values(crenvu_data))) {
  biodiv_index = "critically endangered & \nvulnerable species"
} else {
  biodiv_index = "species richness"
}
```

Loop over iso3 codes to make a list of biodiveristy rasters for each country.
```{r}
biodiv_country_rast <- list()
for (country_iso3 in country_iso3_codes) {
  biodiv_sub <-
    crop(biodiv_data, extent(borders_only_sub[borders_only_sub$iso3 == country_iso3,]))
  biodiv_country_rast[[country_iso3]] <-
    mask(biodiv_sub, borders_only_sub[borders_only_sub$iso3 == country_iso3,])
}
```


```{r}
get_raster_summary <- function(x) {
  tmp1 <- raster::values(x) %>%
    na.omit() %>%
    summary() %>%
    as.vector()
  
  tmp2 <- raster::values(x) %>%
    na.omit() %>%
    sd()
  
  tmp3 <- raster::values(x) %>%
    na.omit() %>%
    length()
  
  return(data.frame(
    names = c(
      "min",
      "first_quart",
      "median",
      "mean",
      "third_quart",
      "max",
      "sd",
      "ncount"
    ),
    biodiv = c(tmp1, tmp2, tmp3)
  ))
}

biodiv_country_moments <-
  lapply(biodiv_country_rast, FUN = get_raster_summary)
```

## Species Richness Analysis
Make a list of countries by median biodiversity.
```{r}
median_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][3]
}
mean_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][4]
}
sd_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][7]
}
ncount_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][8]
}

median_tmp <- lapply(names(biodiv_country_moments), FUN = median_vals)
mean_tmp <- lapply(names(biodiv_country_moments), FUN = mean_vals)
sd_tmp <- lapply(names(biodiv_country_moments), FUN = sd_vals)
ncount_tmp <- lapply(names(biodiv_country_moments), FUN = ncount_vals)

biodiv_moments_df <- data.frame(
  iso_alpha3 = names(biodiv_country_moments),
  biodiv_median = unlist(median_tmp), 
  biodiv_mean = unlist(mean_tmp), 
  biodiv_sd = unlist(sd_tmp), 
  biodiv_ncount = unlist(ncount_tmp)
)

rm(median_tmp, mean_tmp, sd_tmp, ncount_tmp)
```


```{r}
hist_moments <- hist_pathways_moments %>% 
  full_join(biodiv_moments_df, by = "iso_alpha3") %>% 
  filter(!is.na(hist_diff1_ave) & !is.na(biodiv_median) & !is.na(iso_alpha3))

hist_moments <- hist_moments %>% 
  dplyr::filter(biodiv_median > 5)

```

Make some annotation label dfs to show correlation coefficients on the plot facets.
```{r}
plant_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

plant_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
animal_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)
fish_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    # pull(biodiv_mean)
    pull(biodiv_median)
)

corval_labs_pos = c(
  paste0(
    "r = ",
    round(plant_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_pos_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_pos_corr$estimate, 2),
     " (p ~ ",
    signif(fish_prod_pos_corr$p.value, 1),
    ")"
  )
)
corval_labs_neg = c(
  paste0(
    "r = ",
    round(plant_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(plant_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(animal_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_neg_corr$estimate, 2),
    " (p ~ ",
    signif(fish_prod_neg_corr$p.value, 1),
    ")"
  )
)

annotate_text <- data.frame(
  food_group = c("plant products", "animal products", "fish & seafood"),
  corval_labs_pos, 
  corval_labs_neg
)
```

Plot the historical data.
```{r}
p_pos0 <- ggplot(
  data = hist_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    filter(hist_diff1_ave > 0),
  aes(x = hist_diff1_ave,
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, ymax = biodiv_median + biodiv_sd),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick",
              method = "lm",
              fill = NA) +
  theme_bw() +
  theme(
    title = element_text(size = 10),
    legend.position = "top",
    legend.title = element_blank(),
    panel.grid = element_blank()
  ) +
  labs(
    title = "High biodiveristy countries have experienced more rapid speed-ups in historical supply",
    x = "Average FAO historical ROC (kt/yr)",
    y = biodiv_index
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)

p_pos <- p_pos0 +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap(~ food_group)

p_neg0 <- ggplot(
  data = hist_moments %>%
    dplyr::filter(food_group != "fish & seafood") %>%
    dplyr::filter(hist_diff1_ave < 0),
  aes(x = abs(hist_diff1_ave),
      y = biodiv_median)
) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_median - biodiv_sd, 
        ymax = biodiv_median + biodiv_sd
        ),
    colour = scales::muted("blue"),
    # width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick",
              method = "lm",
              fill = NA) +
  theme_bw() +
  theme(
    title = element_text(size = 10),
    legend.position = "top",
    legend.title = element_blank(),
    panel.grid = element_blank()
  ) +
  labs(title = "Slow-downs in historical supply have no correlation with biodiversity",
       x = "Average FAO historical ROC (kt/yr)",
       y = biodiv_index) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  coord_cartesian(xlim = c(-1e-2, 1.5e4)) +
  annotation_logticks(sides = "b", size = 0.4)
p_neg <- p_neg0 +
  geom_text(
    data    = annotate_text %>%
      dplyr::filter(food_group != "fish & seafood"),
    mapping = aes(x = 10, y = 44, label = corval_labs_neg),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap(~ food_group) 
```


```{r, message=FALSE, warning=FALSE}
cowplot::plot_grid(p_pos, p_neg, ncol = 1)
png(filename = "~/Documents/Data/Output/hist_plots_cut.png", width = 600, height = 400)
cowplot::plot_grid(p_pos, p_neg, ncol = 1)
dev.off()
```

####
testing


```{r}
p_path_pos0 <- ggplot(data = pathways_biodiv_moments_em %>% 
                        dplyr::filter(food_group != "fish & seafood") %>% 
                        dplyr::filter(delta_diff1_em > 0),
                      aes(x = delta_diff1_em,
                          y = biodiv_em)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"), size = 0.8, alpha = 0.2) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.2
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.2
  ) +
  geom_smooth(
    colour = scales::muted("grey"),
    method = "lm",
    fill = NA,
    alpha = 0.5, 
    linetype = 2
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank()
  ) +
  labs(tag = "A",
       title = "Speed-ups in pathway gap",
       x = "Difference between future pathway and historical ROC (kt/yr)",
       y = "Critically endangered & vulnerable terrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
  ) +
  geom_text(
    data    = annotate_text %>% 
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(droc_sign > 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  coord_cartesian(xlim = c(1e-3, 1e5))
p_path_pos <- p_path_pos0 +
  geom_point(
    data = pathways_biodiv_moments_em %>% 
                        dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(delta_diff1_em > 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    size = 1,
    shape = 2,
    colour = "firebrick",
    alpha = 0.9
  ) +
  geom_smooth(
    data = pathways_biodiv_moments_em %>% 
      dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(delta_diff1_em > 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    method = "lm",
    colour = "black",
    fill = NA
  ) + 
  facet_grid(diet_plan ~ food_group)

p_path_neg0 <- ggplot(data = pathways_biodiv_moments_em %>% 
                        dplyr::filter(food_group != "fish & seafood") %>%
                        dplyr::filter(delta_diff1_em < 0),
                      aes(x = abs(delta_diff1_em),
                          y = biodiv_em)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"), size = 0.8, alpha = 0.5) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) + 
  geom_smooth(
    colour = scales::muted("grey"),
    method = "lm",
    fill = NA,
    alpha = 0.5, 
    linetype = 2
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 11), 
    title = element_text(size = 12), 
    legend.position = "top",
    legend.title = element_blank()
  ) +
  labs(tag = "B",
       title = "Slow-downs in pathway gap",
       x = "Difference between future pathway and historical ROC (kt/yr)",
       y = "Critically endangered & vulnerable terrestrial species") +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  ) +
  geom_text(
    data    = annotate_text %>% 
      dplyr::filter(food_group != "fish & seafood") %>% 
      dplyr::filter(droc_sign < 0),
    mapping = aes(x = 10, y = 44, label = labz),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  coord_cartesian(xlim = c(1e-3, 1e5))
p_path_neg <- p_path_neg0 +
  geom_point(
    data = pathways_biodiv_moments_em %>% 
                        dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(delta_diff1_em < 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    size = 1,
    shape = 2,
    colour = "firebrick",
    alpha = 0.9
  ) +
  geom_smooth(
    data = pathways_biodiv_moments_em %>% 
                        dplyr::filter(food_group != "fish & seafood") %>%
      dplyr::filter(delta_diff1_em > 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    method = "lm",
    colour = "black",
    fill = NA
  ) +
  coord_cartesian(xlim = c(1e-3, 1e4)) +
  facet_grid(diet_plan ~ food_group)

```

```{r}
cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
# png(filename = "~/Documents/Data/Output/pathways_plots.png", width = 7.5, height = 4.5, units = "in", res = 1000)
# cowplot::plot_grid(p_path_pos, p_path_neg, ncol = 2)
# dev.off()
```



## Total Species Richness Analysis
```{r}
biodiv_data <- speciesrichness_data

# used for ease of making plot labels below
if(identical(raster::values(biodiv_data), raster::values(crenvu_data))) {
  biodiv_index = "critically endangered & \nvulnerable species"
} else {
  biodiv_index = "species richness"
}

```


Loop over iso3 codes to make a list of biodiveristy rasters for each country.
```{r}
biodiv_country_rast <- list()
for (country_iso3 in country_iso3_codes) {
  biodiv_sub <-
    crop(biodiv_data, extent(borders_only_sub[borders_only_sub$iso3 == country_iso3,]))
  biodiv_country_rast[[country_iso3]] <-
    mask(biodiv_sub, borders_only_sub[borders_only_sub$iso3 == country_iso3,])
}
```


Extract some basic data (mean, sd, median, min, max) from each of these.
```{r}
get_raster_summary <- function(x) {
  tmp1 <- raster::values(x) %>%
    na.omit() %>%
    summary() %>%
    as.vector()
  
  tmp2 <- raster::values(x) %>% 
    na.omit() %>% 
    sd()
  if(!is.na(tmp2)) {
    tmp2 <- tmp2
  } else {
    tmp2 <- 0
  }
  
  return(data.frame(
    names = c(
      "min",
      "first_quart",
      "median",
      "mean",
      "third_quart",
      "max",
      "sd"
    ),
    biodiv = c(tmp1, tmp2)
  ))
}

biodiv_country_moments <-
  lapply(biodiv_country_rast, FUN = get_raster_summary)
```

Now that the data has been ordered, proceed to analytics. First, let's make a simple list of countries by median biodiversity.
```{r}
median_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][3]
}
mean_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][4]
}
sd_vals <- function(x) {
  biodiv_country_moments[[x]][[2]][7]
}

median_tmp <- lapply(names(biodiv_country_moments), FUN = median_vals)
mean_tmp <- lapply(names(biodiv_country_moments), FUN = mean_vals)
sd_tmp <- lapply(names(biodiv_country_moments), FUN = sd_vals)

biodiv_moments_df <- data.frame(
  iso_alpha3 = names(biodiv_country_moments),
  biodiv_median = unlist(median_tmp), 
  biodiv_mean = unlist(mean_tmp), 
  biodiv_sd = unlist(sd_tmp)
) %>% 
  tidyr::drop_na()

rm(median_tmp, mean_tmp)
```

### Find pathway ROC gaps
We want to see the difference between transformation pathway and historical values; so first transformation pathways...
```{r}
trans_pathways_moments <- pathways %>%
  dplyr::group_by(iso_alpha3, diet_plan, scenario, food_group, target_year) %>%
  dplyr::filter(!is.na(diff1)) %>%
  dplyr::summarise(
    diff1_ave = mean(diff1, na.rm = TRUE),
    diff1_med = median(diff1, na.rm = TRUE),
    diff1_sdv = sd(diff1, na.rm = TRUE),
    .groups = "drop"
  )
```

...and then historical pathways.
```{r}
hist_pathways_moments <- historical_pathways %>%
  dplyr::group_by(iso_alpha3, food_group)  %>%
  dplyr::summarise(
    hist_diff1_ave = mean(diff1, na.rm = TRUE),
    hist_diff1_med = median(diff1, na.rm = TRUE),
    hist_diff1_sdv = sd(diff1, na.rm = TRUE),
    .groups = "drop"
  )
```

Next, let's take differences (delta) between long-term averages of historical (FAO) and transformation pathways.
```{r}
delta_pathways_moments <- trans_pathways_moments %>%
  dplyr::full_join(hist_pathways_moments, by = c("iso_alpha3", "food_group")) %>%
  dplyr::mutate(
    delta_diff1_ave = diff1_ave - hist_diff1_ave,
    delta_diff1_med = diff1_med - hist_diff1_med
  ) %>%
  dplyr::select(
    iso_alpha3,
    diet_plan,
    scenario,
    food_group,
    target_year,
    delta_diff1_ave,
    delta_diff1_med
  )
```

Merge these data sets.
```{r}
pathways_biodiv_moments_df <- delta_pathways_moments %>%
  dplyr::full_join(biodiv_moments_df, by = "iso_alpha3") %>%
  tidyr::drop_na()
```

`pathways_biodiv_moments_df` contains differences (deltas) between transformation and historical pathway means, which I call "gaps".

### Look at historical ROCs
Historical pathways and biodiversity
```{r}
hist_moments <- hist_pathways_moments %>% 
  full_join(biodiv_moments_df, by = "iso_alpha3") %>% 
  filter(!is.na(hist_diff1_ave) & !is.na(biodiv_median) & !is.na(iso_alpha3))

```

Make some annotation label dfs to show correlation coefficients on the plot facets.
```{r}
plant_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    pull(biodiv_mean)
)
animal_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    pull(biodiv_mean)
)
fish_prod_pos_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    pull(biodiv_mean)
)

plant_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    pull(biodiv_mean)
)
animal_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    pull(biodiv_mean)
)
fish_prod_neg_corr <- cor.test(
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    pull(biodiv_mean)
)

corval_labs_pos = c(
  paste0(
    "r = ",
    round(plant_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(plant_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(animal_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(fish_prod_pos_corr$p.value, 1),
    ")"
  )
)
corval_labs_neg = c(
  paste0(
    "r = ",
    round(plant_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(plant_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(animal_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(fish_prod_neg_corr$p.value, 1),
    ")"
  )
)

annotate_text <- data.frame(
  food_group = c("plant products", "animal products", "fish & seafood"),
  corval_labs_pos, 
  corval_labs_neg
)
```

Plot the historical data.
```{r}
p_pos0 <- ggplot(data = hist_moments %>%
                  filter(hist_diff1_ave > 0),
                aes(x = hist_diff1_ave,
                    y = biodiv_mean)) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mean - biodiv_sd, ymax = biodiv_mean + biodiv_sd),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick", method = "lm") +
  theme_bw() +
  theme(
    title = element_text(size = 10),
    legend.position = "top",
        legend.title = element_blank()) +
  labs(
    title = "High biodiveristy countries have experienced more rapid speed-ups in historical supply",
    subtitle = "Sources: FAO & ICUN Red List",
    x = "Average FAO historical ROC (kt/yr)",
    y = biodiv_index
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^
                                                                 .x))
  )
p_pos <- p_pos0 +
  geom_text(
    data    = annotate_text,
    mapping = aes(x = 10, y = 40, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap( ~ food_group)

p_neg0 <- ggplot(data = hist_moments %>%
                  filter(hist_diff1_ave < 0),
                aes(x = abs(hist_diff1_ave),
                    y = biodiv_mean)) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(
      xmin = hist_diff1_ave - hist_diff1_sdv,
      xmax = hist_diff1_ave + hist_diff1_sdv
    ),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mean - biodiv_sd, ymax = biodiv_mean + biodiv_sd),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick", method = "lm") +
  theme_bw() +
  theme(
    title = element_text(size = 10), 
    legend.position = "top",
        legend.title = element_blank()) +
  labs(
    title = "Slow-downs in historical supply have no correlation with biodiversity",
    subtitle = "Sources: FAO & ICUN Red List",
    x = "Average FAO historical ROC (kt/yr)",
    y = biodiv_index
  ) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
      tmp <- abs(x)
      10 ^ tmp
    }),
    labels = scales::trans_format("log10", scales::math_format(-10 ^
                                                                 .x))
  )
p_neg <- p_neg0 +
  geom_text(
    data    = annotate_text,
    mapping = aes(x = 10, y = 40, label = corval_labs_neg),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) +
  facet_wrap( ~ food_group)
```


```{r}
cowplot::plot_grid(p_pos, p_neg, ncol = 1)
```

This shows that large increases in the ROC of production (speed-ups) are more common in high-biodiversity countries, and decreases (down-shifts) are not correlated at all.

Is this different for countries that have signed up to the HAC pledge?
```{r}
hac_hist_moments <- hist_moments %>% 
  dplyr::filter(iso_alpha3 %in% hac_codes)
```

Run the correlations again.
```{r}
plant_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "plant products") %>% 
    pull(biodiv_mean)
)
animal_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "animal products") %>% 
    pull(biodiv_mean)
)
fish_prod_pos_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(hist_diff1_ave)) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave > 0 & food_group == "fish & seafood") %>% 
    pull(biodiv_mean)
)

plant_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "plant products") %>% 
    pull(biodiv_mean)
)
animal_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "animal products") %>% 
    pull(biodiv_mean)
)
fish_prod_neg_corr <- cor.test(
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    mutate(hist_diff1_ave = log(abs(hist_diff1_ave))) %>% 
    pull(hist_diff1_ave), 
  hac_hist_moments %>% 
    filter(hist_diff1_ave < 0 & food_group == "fish & seafood") %>% 
    pull(biodiv_mean)
)

corval_labs_pos = c(
  paste0(
    "r = ",
    round(plant_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(plant_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(animal_prod_pos_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_pos_corr$estimate, 2),
    " (p-value = ",
    signif(fish_prod_pos_corr$p.value, 1),
    ")"
  )
)
corval_labs_neg = c(
  paste0(
    "r = ",
    round(plant_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(plant_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(animal_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(animal_prod_neg_corr$p.value, 1),
    ")"
  ),
  paste0(
    "r = ",
    round(fish_prod_neg_corr$estimate, 2),
    " (p-value = ",
    signif(fish_prod_neg_corr$p.value, 1),
    ")"
  )
)

annotate_text <- data.frame(
  food_group = c("plant products", "animal products", "fish & seafood"),
  corval_labs_pos, 
  corval_labs_neg
)
```

Blue triangle-shaped points are over-plotted to represent HAC signatories.
```{r}
p_pos1 <- p_pos0 + 
 geom_point(data = hac_hist_moments %>%
    dplyr::filter(hist_diff1_ave > 0), 
    aes(x = hist_diff1_ave,
      y = biodiv_mean), 
    size = 2, 
    shape = 2, 
    colour = "blue") + 
  geom_smooth(data = hac_hist_moments %>%
    dplyr::filter(hist_diff1_ave > 0), 
    mapping = aes(x = hist_diff1_ave,
      y = biodiv_mean), 
    method = "lm", colour = "black", linetype = 2, fill = "lightblue") +
  geom_text(
    data    = annotate_text,
    mapping = aes(x = 10, y = 800, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) + 
  coord_cartesian(xlim = c(-1e-2, 1.5e4), ylim = c(0,800)) + 
  facet_wrap(~ food_group)

p_neg1 <- p_neg0 + 
 geom_point(data = hac_hist_moments %>%
    dplyr::filter(hist_diff1_ave < 0), 
    aes(x = abs(hist_diff1_ave),
      y = biodiv_mean), 
    size = 2, 
    shape = 2, 
    colour = "blue") + 
  geom_smooth(data = hac_hist_moments %>%
    dplyr::filter(hist_diff1_ave < 0), 
    mapping = aes(
      x = abs(hist_diff1_ave),
      y = biodiv_mean), 
    method = "lm", colour = "black", linetype = 2, fill = "lightblue") +
  geom_text(
    data    = annotate_text,
    mapping = aes(x = 10, y = 800, label = corval_labs_pos),
    hjust   = 0.4,
    vjust   = 0.9,
    size = 3
  ) + 
  coord_cartesian(xlim = c(-1e-2, 1.5e4), ylim = c(0,800)) + 
  facet_wrap(~ food_group)
```

The over-plotted open triangles correspond to countries that are HAC signatories. The others are not. The dashed line is the linear fit to the HAC countries; the solid line is to all countries.
```{r}
cowplot::plot_grid(p_pos1, p_neg1, ncol = 1)
```

The correlation with speed-ups appears to go away entirely; it's still uncorrelated for down-shifts.


### Dependence of slopes with target year
```{r}
pathways_biodiv_moments_em <- pathways_biodiv_moments_df %>% 
  dplyr::group_by(iso_alpha3, diet_plan, food_group, target_year) %>% 
  dplyr::summarise(delta_diff1_em = mean(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mn = min(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mx = max(delta_diff1_ave, na.rm = TRUE), 
                   biodiv_em = mean(biodiv_mean, na.rm = TRUE), 
                   biodiv_mn = biodiv_em - mean(biodiv_sd, na.rm = TRUE), 
                   biodiv_mx = biodiv_em + mean(biodiv_sd, na.rm = TRUE), 
                   .groups = "drop")
```

```{r}
p_path_pos0 <- ggplot(data = pathways_biodiv_moments_em %>%
         dplyr::filter(delta_diff1_em > 0) %>% 
           dplyr::filter(diet_plan == "FDA-SDP"),
       aes(x = delta_diff1_em,
           y = biodiv_em)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_smooth(colour = "firebrick", method = "lm") + 
  theme_bw() +
  theme(legend.position = "top",
        legend.title = element_blank()) +
  labs(title = "Speed-ups in pathway gap with biodiversity",
       x = "Average FAO historical ROC (kt/yr)",
       y = biodiv_index) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
  )
```

Plot the result
```{r}
p_path_pos <- p_path_pos0 +
  facet_wrap(food_group ~ target_year, ncol = 5)
```

How do the slopes of the regression lines change as a function of target year?
```{r}
p_path_pos_dat <- ggplot_build(p_path_pos)
```

```{r}
pathways_fit_df <- p_path_pos_dat$data[[5]]
slopes_by_target_year <- pathways_fit_df %>%
  dplyr::group_by(PANEL) %>%
  dplyr::mutate(dx = x - lag(x, 1),
                dy = y - lag(y, 1)) %>%
  dplyr::summarise(dx = mean(dx, na.rm = TRUE),
                   dy = mean(dy, na.rm = TRUE)) %>%
  dplyr::mutate(slope = dy / dx,
                target_year = ifelse(PANEL == 1, 2045,
                                     ifelse(
                                       PANEL == 2, 2050,
                                       ifelse(PANEL == 3, 2055,
                                              ifelse(PANEL == 4, 2060,
                                                     2065))
                                     ))) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(target_year, slope)
```

Plotting this roughly shows that the slope stays pretty constant until 2055, after which it drops.
```{r}
ggplot(data = slopes_by_target_year, 
       aes(x = target_year, 
           y = slope)) + 
  geom_line() + 
  theme_minimal() + 
  labs(
    x = "shift target year", 
    y = "ave. species richness / pathway t food-feed demand / yr"
  )
```
### Plot for future ROC gap
There appears to be no significant association with target year, but there is with food group.
```{r}
pathways_biodiv_moments_em <- pathways_biodiv_moments_df %>% 
  dplyr::group_by(iso_alpha3, diet_plan, food_group) %>% 
  dplyr::summarise(delta_diff1_em = mean(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mn = min(delta_diff1_ave, na.rm = TRUE), 
                   delta_diff1_mx = max(delta_diff1_ave, na.rm = TRUE), 
                   biodiv_em = mean(biodiv_mean, na.rm = TRUE), 
                   biodiv_mn = biodiv_em - mean(biodiv_sd, na.rm = TRUE), 
                   biodiv_mx = biodiv_em + mean(biodiv_sd, na.rm = TRUE), 
                   .groups = "drop")
```

```{r}
p_path_pos0 <- ggplot(data = pathways_biodiv_moments_em %>%
                        dplyr::filter(delta_diff1_em > 0),
                      aes(x = delta_diff1_em,
                          y = biodiv_em)) +
  geom_vline(xintercept = 0, alpha = 0.9) +
  geom_point(colour = scales::muted("blue"), alpha = 0.5) +
  geom_errorbar(
    aes(xmin = delta_diff1_mn,
        xmax = delta_diff1_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_errorbar(
    aes(ymin = biodiv_mn,
        ymax = biodiv_mx),
    colour = scales::muted("blue"),
    width = .2,
    alpha = 0.5
  ) +
  geom_smooth(
    colour = "firebrick",
    method = "lm",
    fill = NA,
    alpha = 0.9
  ) +
  theme_bw() +
  theme(legend.position = "top",
        legend.title = element_blank()) +
  labs(title = "Speed-ups in pathway gap with biodiversity", 
       subtitle = "Pathways projected up to year 2065", 
       x = "Average FAO historical ROC (kt/yr)",
       y = biodiv_index) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x)
      10 ^ x),
    labels = scales::trans_format("log10", scales::math_format(10 ^ .x))
  )
```


Now look at just HAC signatories.
```{r}
p_path_pos0 +
  geom_point(
    data = pathways_biodiv_moments_em %>%
      dplyr::filter(delta_diff1_em > 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    size = 2,
    shape = 2,
    colour = "red",
    alpha = 0.9
  ) +
  geom_smooth(
    data = pathways_biodiv_moments_em %>%
      dplyr::filter(delta_diff1_em > 0) %>%
      dplyr::filter(iso_alpha3 %in% hac_codes),
    aes(x = abs(delta_diff1_em),
        y = biodiv_em),
    method = "lm",
    colour = "black",
    linetype = 1,
    fill = NA, 
    alpha = 0.9
  ) +
  coord_cartesian(xlim = c(1e-3, 1e4)) +
  facet_grid(food_group ~ diet_plan)
```








Pearson's correlation coefficient for the test data
```{r}
cor.test(test$delta_diff1_ave, test$biodiv_mean)
```

### Across all pathways
Pathways are defined by combinations of diets, SSP scenarios, food groups and target years. Let's examine correlations across all combinations. Start with a combination matrix.
```{r}
combination_matrix <- expand.grid(
  diet_plan = c("CNS-CFP", "ELA-PHD", "FDA-SDP"),
  scenario = c("SSP1", "SSP2", "SSP3", "SSP4", "SSP5"),
  food_group = c("animal products", "fish & seafood", "plant products"),
  target_year = seq(2045, 2065, by = 5)
)

combination_matrix <- combination_matrix %>% 
  dplyr::mutate(index = 1:225) # the matrix is 225 rows long
```

These countries have inordinately large ROC gaps. Negative gaps, for the USA and CHN, are a consequence of the historical supply surplus in the US and likely future population drop in China. Positive gaps are due to India, Nigeria and Pakistan's dependence on food imports (low historical supply) and very high future demand from population growth.
```{r}
outlier_countries <- c("CHN", "IND", "NGA", "PAK", "USA")
```

First look at positive pathway ROC gaps (i.e., transformation pathway ROC is greater than historical ROC).
```{r}
for(index in 1:225) {
  tmp0 <- combination_matrix[index, ]
  tmp1 <- pathways_biodiv_moments_df %>% 
    dplyr::filter(delta_diff1_ave >= 0) %>% 
    dplyr::filter(
      diet_plan == as.character(tmp0$diet_plan) &
        scenario == as.character(tmp0$scenario) &
        food_group == as.character(tmp0$food_group) &
        target_year == as.double(tmp0$target_year)
    )
  tmp2 <- cor.test(tmp1$delta_diff1_ave, tmp1$biodiv_mean)
  tmp3 <- data.frame(
    index = index, 
    teststat = tmp2$statistic,
    df = tmp2$parameter, 
    corr = tmp2$estimate, 
    pval = tmp2$p.value
  )
  if(index == 1) {
    pos_corr_tests <- tmp3
  } else {
    pos_corr_tests <- pos_corr_tests %>% 
      dplyr::bind_rows(tmp3)
  }
}
```

Next, negative pathway ROC gaps (i.e., transformation ROC is less than historic ROC).
```{r}
for(index in 1:225) {
  tmp0 <- combination_matrix[index, ]
  tmp1 <- pathways_biodiv_moments_df %>% 
    dplyr::filter(delta_diff1_ave < 0) %>% 
    dplyr::filter(
      diet_plan == as.character(tmp0$diet_plan) &
        scenario == as.character(tmp0$scenario) &
        food_group == as.character(tmp0$food_group) &
        target_year == as.double(tmp0$target_year)
    )
  tmp2 <- cor.test(tmp1$delta_diff1_ave, tmp1$biodiv_mean)
  tmp3 <- data.frame(
    index = index, 
    teststat = tmp2$statistic,
    df = tmp2$parameter, 
    corr = tmp2$estimate, 
    pval = tmp2$p.value
  )
  if(index == 1) {
    neg_corr_tests <- tmp3
  } else {
    neg_corr_tests <- neg_corr_tests %>% 
      dplyr::bind_rows(tmp3)
  }
}
```

Use the p-value to filter significant correlation values.
```{r}
pos_corr_tests_filtered <- pos_corr_tests %>% 
  dplyr::filter(pval <= 0.05) %>% 
  dplyr::left_join(combination_matrix, by = "index") %>% 
  dplyr::arrange(-corr)

neg_corr_tests_filtered <- neg_corr_tests %>% 
  dplyr::filter(pval <= 0.05) %>% 
  dplyr::left_join(combination_matrix, by = "index") %>% 
  dplyr::arrange(-corr)
```

There are just 5 cases (from `neg_corr_tests_filtered`) in which the correlation estimate was significant (p-value between 0.03 and 0.047), but all correlations are negative: there are special cases in the CNS and ELA diets in which more feasible shifts occur in high-biodiversity countries.

```{r}
colz <-
  c(
    "animal products" = "#F17EB8",
    "fish & seafood" = "#489ED3",
    "plant products" = "#EEAF35"
  )
labz <-
  c("Animal Products", "Fish and Seafood", "Plant Products")

p_corr <- ggplot(
  data = pos_corr_tests_filtered %>%
    dplyr::mutate(diet_plan = ifelse(
      diet_plan == "ELA-PHD",
      "ELA",
      ifelse(diet_plan == "CNS-CFP",
             "CNS",
             "USDA")
    )),
  mapping = aes(x = target_year,
                y = corr)
) +
  geom_point(aes(shape = food_group,
                 colour = food_group), 
             size = 2) +
  scale_color_manual(labels = labz, 
                     values = colz) +
  scale_shape_manual(labels = labz, 
                     values = c(15, 16, 17)) +
  theme_bw() +
  theme(
    legend.title = element_blank(),
    legend.position = "top",
    axis.text.x = element_text(
      angle = 90,
      hjust = 0.5,
      vjust = 0.5,
      size = 7
    )
  ) +
  labs(
    title = "Food system speed-ups are less feasible for high biodiversity countries",
    subtitle = "Correlation (p < 0.05) between mean biodiveristy index and food shift feasibility gap",
    x = "target year",
    y = "Pearson's correlation coefficient"
  ) +
  facet_grid(diet_plan ~ scenario)

p_corr
```



