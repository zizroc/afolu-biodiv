...
The over-plotted open triangles correspond to countries that are HAC signatories. The others are not. The dashed blue line is the linear fit to the HAC countries; the red line is to all countries.
```{r}
cowplot::plot_grid(p_pos1, p_neg1, ncol = 1)
```

The correlation with up-shifts appears to weaken or go away; it's unchanged for down-shifts.
...
dat_tmp <- pathways_biodiv_moments_df %>% 
  dplyr::select(-delta_diff1_med, -biodiv_median, -biodiv_mean, -biodiv_sd) %>% 
  dplyr::filter(delta_diff1_ave > 0)
# %>% 
...
anova_tmp <- aov(delta_diff1_ave ~ target_year, data = dat_tmp)
...
for(iso in countries) {
  ...
  if(!is.na(unlist(anova_results)[9])) {
     pval_tmp <- unlist(anova_results)[9]
  } else {
    pval_tmp <- NA
  }
  tmp <- data.frame(
...
recoded_pathways_df <- pathways_biodiv_moments_df %>% 
  ...
  dplyr::select(-diet_plan, -scenario, -food_group, -delta_diff1_med, -biodiv_median, -biodiv_mean, -biodiv_sd)
```
