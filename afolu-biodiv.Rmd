...
p_neg0 <- ggplot(
  ...
  labs(title = "Down-shifts in historical supply have no correlation with biodiversity",
       x = "Average FAO historical ROC (kt/yr)",
       y = biodiv_index) +
  scale_x_log10(
    breaks = scales::trans_breaks("log10", function(x) {
...
p_hac_pos <- ggplot(hac_correlations %>% 
  ...
  labs(
    tag = "A", 
    title = "For up-shifts in pathway gap", 
...
p_hac_neg <- ggplot(hac_correlations %>% 
  ...
  labs(
    tag = "B", 
    title = "For down-shifts in pathway gap", 
    y = "Pearson's correlation coefficient"
  ) + 
  facet_grid(diet_plan ~ food_group)
