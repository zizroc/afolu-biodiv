...
We want to see the difference between transformation pathway and historical values.
```{r}
...
Let's examine correlations across all combinations. Start with a combination matrix.
```{r}
...
```{r}
p_corr <- ggplot(data = pos_corr_tests_filtered, 
  ...
  labs(
    ...
    subtitle = "Correlation (p < 0.05) between mean biodiveristy index and food pathway gap",
    x = "target year", 
