...
```{r}
path_tmp <- trans_path_biodiv_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(diff1_em < 0) %>%
  dplyr::select(diet_plan, food_group, diff1_em)
# %>% 
#   dplyr::group_by(diet_plan, food_group) %>% 
#   tidyr::nest(path_data = diff1_em)

hist_tmp <- hist_pathways_moments %>%
  dplyr::filter(food_group != "fish & seafood") %>%
  dplyr::filter(hist_diff1_ave < 0) %>%
  dplyr::select(food_group, hist_diff1_ave)
# %>% 
#   dplyr::group_by(food_group) %>% 
#   tidyr::nest(hist_data = hist_diff1_ave)

hist_path_tmp <- path_tmp %>% 
  dplyr::full_join(hist_tmp)
...
Take differences.
```{r}
p_neg_dat <- ggplot_build(p_path_dens_neg)

neg_dat1 <- p_neg_dat$data[[2]] %>% 
  dplyr::mutate(pathway = ifelse(fill == "#3A3A98", 
                                 "future", 
                                 ifelse(fill == "#832424", 
                                        "historical", 
                                        NA))) %>% 
  dplyr::select(x, y, PANEL, pathway)

neg_dat2 <- p_neg_dat$data[[3]] %>% 
  dplyr::mutate(pathway = ifelse(fill == "#3A3A98", 
                                 "future", 
                                 ifelse(fill == "#832424", 
                                        "historical", 
...
neg_dat <- neg_dat1 %>% 
  dplyr::mutate(sign = ifelse(pathway == "future", 
                                 1, 
                                 ifelse(pathway == "historical", 
                                        -1, 
                                        NA))) %>% 
  dplyr::mutate(
    x_prime = round(x, 1), 
  ...
  dplyr::full_join(neg_dat2) %>% 
  dplyr::group_by(x_prime, PANEL) %>% 
  dplyr::summarise(y_diff = sum(y_prime), 
                   .groups = "drop")
